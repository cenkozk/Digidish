...
function QRLinks(props) {
  ...
  const qrCode = new QRCodeStyling({
    width: 2048,
    height: 2048,
    data: `${window.location.origin}/${props.restaurantId}`,
    margin: 0,
  ...
  return (
    <div>
      {props.restaurantId ? (
        <div>
          <div className=" flex flex-col ml-auto-auto scale-90 pb-6">
            <div class="bg-white border w-fit mr-auto border-gray-200 rounded-lg p-8 scale-90">
            ...
            <div class="bg-white border w-fit mr-auto border-gray-200 rounded-lg p-8  scale-90">
              ...
              <a
                href={`${window.location.origin}/${props.restaurantId}`}
                target="_blank"
                rel="noopener noreferrer"
                className=" text-orange-400 text-2xl"
              >{`${window.location.origin}/${props.restaurantId}`}</a>
            </div>
          </div>
        </div>
      ) : (
        <div className=" flex flex-col justify-center items-center scale-90 h-full mt-48 pb-6">
          <div class="bg-white border border-gray-200  rounded-lg p-8  scale-90">
            <h1 class="text-gray-900  text-3xl md:text-5xl font-extrabold mb-2">
              Menu Creation Required
            </h1>
            <p class="text-lg font-normal text-gray-500 mb-6">
              Before accessing QR codes and links, please navigate to the 'Menu'
              tab and create your menu first.
            </p>
          </div>
        </div>
      )}
